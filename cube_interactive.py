...
class Cube:
    ...
    def __init__(self, N=3, steps=[], iii=100,plastic_color=None, face_colors=None):
        self.N = N
        ...
        self.rub = rubic()
        if len(steps)==0:
            steps = self.rub.mix(iii,change_cub=False)
        for s in steps:
            self.rub.rotate_by_name(s)
            if len(s) == 2:
                if s[1] == '2':
                    self.rotate_face(s[0])
                    self.rotate_face(s[0])
                elif s[1]=="'":
                    self.rotate_face(s[0],-1)
            else:
                self.rotate_face(s)
        self.mv1, self.mv2, self.mv3, self.mv4=[],[],[],[]
        self.mv5, self.mv6, self.mv7 = [],[],[]
        stepone().solver(self.rub, self.mv1)
        steptwo().solver(self.rub, self.mv2)
        stepthree().solver(self.rub, self.mv3)
        stepfore().solver(self.rub, self.mv4)
        stepfive().solver(self.rub, self.mv5)
        stepsix().solver(self.rub, self.mv6)
        stepseven().solver(self.rub, self.mv7)
    ...
    def draw_interactive(self):
        ...
        fig.add_axes(InteractiveCube(cube=self,my_cube=(self.mv1,self.mv2,\
                                                   self.mv3,\
                                                   self.mv4,\
                                                   self.mv5,\
                                                   self.mv6,\
...
class InteractiveCube(plt.Axes):
    def __init__(self, my_cube=None,cube=None,
                 interactive=True,
                 view=(0, 0, 10),
    ...
    def _initialize_widgets(self):
        ...
        #
        self._ax_step = self.figure.add_axes([0.35, 0.05, 0.2, 0.075])
        self._btn_step = widgets.Button(self._ax_step, 'Step')
        self._btn_step.on_clicked(self._solve_step)
    ...
    def suport(self, mv):
        for m in mv:
            if len(m) == 2:
                if m[1] == '2':
                    self.rotate_face(m[0], 2, 0, steps=3)
                elif m[1]=="'":
                    self.rotate_face(m[0],-1)
            else:
                self.rotate_face(m, 1)


    def _solve_step(self,*args):
        if len(self.mv1) != 0:
            self.suport(self.mv1)
            self.mv1=[]
            return
        if len(self.mv2) != 0:
            self.suport(self.mv2)
            self.mv2 = []
            return
        if len(self.mv3) != 0:
            self.suport(self.mv3)
            self.mv3 = []
            return
        if len(self.mv4) != 0:
            self.suport(self.mv4)
            self.mv4 = []
            return
        if len(self.mv5) != 0:
            self.suport(self.mv5)
            self.mv5 = []
            return
        if len(self.mv6) != 0:
            self.suport(self.mv6)
            self.mv6 = []
            return
        if len(self.mv7) != 0:
            self.suport(self.mv7)
            self.mv7 = []
            return
...
def main_gui(steps):
    if isinstance(steps, list):
        ...
        c = Cube(3, steps=steps)

    else:
        c = Cube(3, iii=int(steps))
